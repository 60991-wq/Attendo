<template>
    <div class="p-6 space-y-6">
      ...
      <!-- Liste des UE -->
      <table class="w-full shadow rounded overflow-hidden">
        <thead>
          <tr class="bg-gray-900 text-white">
            <th class="text-left p-2">UE</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="ue in sessionUes" :key="ue.id" class="border">
            <td class="p-2 text-blue-600">{{ ue.code }}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="mt-4">
        ...
        <div class="flex items-center space-x-2">
          ...
          <select v-model="selectedUeId" class="border p-2 rounded">
            ...
            <option v-for="ue in availableUes" :key="ue.id" :value="ue.id">
              {{ ue.code }}
            </option>
          </select>
          <button
